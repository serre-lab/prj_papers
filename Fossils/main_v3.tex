...
classification, artificial intelligence (AI) is well-suited to extract subtle diagnostic patterns 
that may elude human perception. However, applying standard AI approaches to fossil material faces 
...
images (cleared and X-rayed) and trains a deep neural network using multi-scale representations that capture features from 
venation patterns to overall leaf morphology, with triplet-loss embedding to align the two domains. 
...
Remarkably, it maintains a 73.4\% F1-score even when fossil families are entirely excluded from 
...
pathway to unlock large-scale paleobotanical ``dark data''---the vast collections of unidentified 
...
:
\setcounter{table}{0} % Ensure table counter is at 0 for first supplementary table
\begin{table}[ht!]
\centering
{%
