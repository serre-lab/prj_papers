...
\TS{Gregoire -- intro needs to be SHORT. 8 pages total and no experiments section yet. Aim for 3 tight paragraphs, half a column max. Here is the story and the refs -- you write the prose and I will tighten.}
...
\TS{notation pass? text says bold vectors/operators, but most eqs use non-bold $x,h,y,A,B,C$. pick one convention?}
...
\TS{$\widetilde{A}$ / $\widehat{A}$ introduced here but only defined later? add quick one-line definition here (and maybe $\widetilde{B},\widetilde{C},\widehat{B},\widehat{C}$ too)?}
...
\TS{define $T$ explicitly here too? (sequence length in time).}
...
\TS{same notation issue here: ZOH uses bold ($\bm A,\bm B,\bm I$), nearby lines use non-bold ($A,B,I$). keep one style?}

\TS{ZOH: $(\Delta A)^{-1}$ assumes $A$ invertible?}
...
\TS{Fixed your cite key -- I went ahead and populated references.bib with entries for all the papers we need. Keys follow the convention in my Zotero (firstauthorlastname+year). Check that yours match!}
%
...
\TS{\textbf{MOVE}: the ``Spectral SSMs'' and ``Diagonal SSSMs'' paragraphs below should not be a standalone section. Move them into the ``Convolutional State Space Models'' section, right where you resolve Challenge A + B with the spectral formulation. Introduce the Fourier diagonalization there as the solution, not here as background. See the structure comment at the top of \S3 for the full plan.}
...
\TS{Fourier $F$ complex in general. ok with complex params? or real-FFT/conjugate sym?}
...
\TS{"$Q_c$ via diagonalization": blocks diagonalizable over $\mathbb C$ assumed? otherwise Schur?}
...
%
Indeed, discrete-space convolutions correspond to linear operators with a block-circulant with circulant blocks (BCCB) structure, which are diagonalized by the Fourier transform. We get the following ConvSSM with $d = h w c_\mathrm{in}$-dimensional input and output, and $N = h w c$-dimensional hidden state:
%
\begin{align}
\label{eq:convssm:A}
...
\TS{\textbf{KEEP TOGETHER}: after restructuring, the three propositions below (SSM scan works $\to$ spatial ConvSSM explodes $\to$ spectral ConvSSM recovers $O(T \log T)$) should follow immediately after the spectral formulation above. The narrative arc is: here is the problem, here is why naive approaches break, here is why our approach works. Do not separate them.}

\subsection{Complexity analysis}
\label{sec:complexity_analysis}

\paragraph{} We focus our analysis on the parallel scan algorithm. We compare in \cref{app:TODO} the parallel scan to the matrix-multiplication based algorithm of SSD \citep{dao2024} across a wide range of temporal and spatial regimes.
...
\TS{same ref issue here? for spectral case should this point to Eq. \{M/+ /0\}\_SSM\_spectral instead of SSM ones?}
\[
    W(\mathrm{Scan}) = O(T), \qquad T_\infty(\mathrm{Scan}) = O(\log(T))
\]
\end{proposition}
...
\TS{\textbf{RENAME}: ``Final Model'' is not a section title. Call this something like ``The Spectral ConvSSM'' or ``Model Architecture.'' This is the practical model spec that a reader needs to reimplement -- give it a real name.}
...
\TS{notation: should this be $\oplus_{j=0}^i a_j$ (not $\sum$), since operation is generic monoid?}
...
\TS{are we assuming $T=2^L$ here? if not, pad + mention edge handling?}
...
\TS{$I$ here = $I_d$? maybe say explicitly.}
\begin{align}
\label{eq:M_SSM}
...
\TS{$\odot$ in $(A_b\odot A_a)$ = matrix product or Hadamard? later you say element-wise—consistent?}
...
\end{align}
\TS{for Conv$_1$, is $I$ the Dirac/identity kernel? maybe define explicitly.}
...
\TS{Are these refs right? Cref cites SSM eq’s, but monoid is Conv$_1$ above?}
...
\label{eq:0_SSM_spectral}
    &e\coloneqq(I, 0)
\end{align}
\TS{Spectral ConvSSM: $\odot$ as Hadamard? if $\Lambda_c$ blocks not diagonal, should be block mult?}
