...
\thomas{Consider reframing this definition from a dynamical systems / comp neuro perspective rather than graph-theoretic. As written, the graph $(V,E)$ and weights $w_{uv}$ are introduced but $F$ is left completely general, so the graph is formally redundant until you specialize to the linear case below. Suggestion: start from the state equation $h'(t) = F(h(t), x(t))$ and let the connectivity \emph{emerge} as a structural property of $F$---a connection $j \to i$ exists iff $\partial F_i / \partial h_j \not\equiv 0$. This way the graph is derived from the dynamics. Benefits: (1) connects directly to the SSM linearization in Part~II ($F$ linear $\Rightarrow$ $h' = Ah + Bx$, sparsity of $A$ is the graph); (2) stability analysis follows naturally from eigenvalues of the Jacobian; (3) better framing for ICLR audience who think in RNNs and state equations. Keep the graph perspective for the connectivity taxonomy (FF/FB/lateral, circulant, block-sparse)---that is where it genuinely helps.}
