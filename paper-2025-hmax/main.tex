...
Here we use \emph{retinal scale tolerance}---the ability to preserve object identity across changes in retinal size---as a mechanistic probe of ventral-stream models. Humans can generalize across large size changes after minimal exposure \citep{Han2020}, and ventral-stream neurons exhibit tolerant codes that remain stable across changes in size \citep{Logothetis1995,DiCarlo2012}, including curvature tuning in area V4 \citep{PasupathyPopovkinaKim2020V4Review}. These signatures provide constraints that go beyond single operating-point accuracy or predictivity.


Classic work by Hubel and Wiesel characterized a simple feedforward circuit in V1 in which selectivity and limited tolerance emerge over a single step: oriented simple cells respond at specific retinal positions, and complex cells pool over similarly tuned simple-cell inputs to gain tolerance to small shifts in position and phase \citep{HubelWiesel1962,HubelWiesel1977}. This V1 motif later inspired hierarchical models of the ventral stream (including HMAX) that repeat stages of feature selectivity and pooling to progressively increase tolerance to identity-preserving transformations. In these models, pooling is most effective when it is local along a stimulus dimension that is laid out topographically. Retinotopy provides that organization for position, enabling local pooling over neighboring locations. Extending the same idea to retinal size requires an explicit multi-scale representation: changing object size largely shifts its dominant spatial frequencies, so pooling over nearby scales is closely related to pooling over nearby spatial-frequency channels \citep{Witkin1983,Koenderink1984,Lindeberg1994,Lindeberg1998}. Consistent with this link, spatial-frequency preferences exhibit systematic organization in primate V1 and extrastriate cortex \citep{Tootell1988SF,Nauhaus2012,LuRoe2007,Lu2018V1V4,Zhang2023SFV2V4}. More generally, scale-space theory provides theoretical motivation for representing images across multiple scales when constructing size-tolerant features \citep{Lindeberg2013}.
...
At the same time, the recent deep-learning revolution has made foundational contributions to neuroscience by providing high-performing, fully specified computational models and a practical ``goal-driven'' methodology for testing models against neural data at scale (reviewed in \citep{Serre2019GoodBadUgly}). In this context, it is notable that bringing an HMAX-family model back into a contemporary performance regime required comparatively little change to the core ventral-stream motif, and instead relied primarily on improvements that make hierarchical architectures learnable and scalable---trainable filters, the use of smaller stacked kernels, residual connections, and modern optimization and regularization. This suggests that a substantial fraction of recent progress in deep vision may stem from advances in trainability and optimization infrastructure as much as from new representational principles, motivating renewed evaluation of biologically inspired inductive biases under modern training methods.
...
Complementary evidence comes from direct human--model comparisons of transformation tolerance along the human ventral stream. Xu and Vaziri-Pashkam quantified multiple tolerance measures in human fMRI and compared them to standard CNNs, reporting that tolerance increases along the ventral hierarchy in humans but that the corresponding profiles are not reliably reproduced by ImageNet-trained networks \citep{XuVaziriPashkam2022Tolerance}. Although their stimuli used isolated objects pasted on uniform black backgrounds---a design choice that may reduce segmentation and contextual demands and could affect the absolute magnitude of some tolerance estimates---the qualitative human--model dissociation is consistent with our core message: high performance and conventional predictivity can mask systematic differences in how transformation tolerance is represented.

This point is sharpened by recent work on representational quality in deep learning. Kumar and colleagues argue that SGD can produce \emph{fractured entangled representations} (FER), in which task-relevant factors are split across many idiosyncratic, redundantly entangled features even when task performance is high \citep{KumarCluneLehmanStanley2025FER}. Together with broader reviews of what successful visual neural networks learn \citep{HarShalomWeiss2025VNNReview}, FER underscores that benchmark success does not guarantee a well-organized or human-like representational geometry. Targeted perturbation tests that probe specific computational signatures---such as scale tolerance---therefore provide a complementary route to mechanistic validation beyond correlational alignment.
