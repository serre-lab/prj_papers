...
%%=============================================================%%
\abstract{
How the primate ventral visual stream builds invariant object representations remains a central problem. Goal-driven, task-optimized deep networks achieve high recognition accuracy and fit ventral-stream responses better than earlier mechanistic models, but predictive alignment alone does not specify mechanism. We use tolerance to retinal size as a mechanistic probe. Using large-scale object recognition to train and performance-match models, we treat this regime as a way to learn generic size-tolerant circuitry. After training, the HMAX-family model generalizes size tolerance to unfamiliar object identities, whereas performance-matched convolutional networks show tolerance that weakens for unfamiliar objects at novel sizes. The same dissociation appears in human cross-size discrimination after minimal exposure and in macaque inferotemporal recordings: neural mappings fit at one size generalize across sizes for the pooling model but not for standard convolutional networks. Intermediate representations capture scale-normalized curvature coding reported in macaque V4. These results show that conventional benchmarks can obscure distinct computational strategies.
}
...
A central goal of systems neuroscience is to explain how the primate ventral visual stream transforms retinal inputs into representations that support rapid and invariant object recognition. Classic work by Hubel and Wiesel established a foundational computational motif for early vision: receptive fields increase in complexity and tolerance through alternating stages of selectivity and pooling. In primary visual cortex (V1), simple cells exhibit orientation selectivity at specific retinal positions, whereas complex cells pool over similarly tuned inputs to achieve tolerance to position and phase \citep{HubelWiesel1962}. This coupled progression of selectivity and invariance has since served as a conceptual blueprint for hierarchical accounts of object recognition in the ventral stream.

Anatomical and physiological studies later reinforced this view by demonstrating that ventral-stream areas form a serially organized hierarchy (V1, V2, V4, inferotemporal cortex; IT) \citep{FellemanVanEssen1991}, accompanied by increasing complexity of preferred stimuli and increasing tolerance to identity-preserving transformations such as changes in position and scale \citep{Logothetis1995,DiCarlo2012}. These findings motivated a class of hierarchical computational models that instantiate repeated alternations of feature selectivity and pooling to build invariant representations. Fukushima’s Neocognitron provided an early computational realization of this principle through cascaded layers of simple- and complex-like units \citep{Fukushima1980}, and subsequent theoretical work proposed that invariance could be learned from natural transformation sequences via temporal association mechanisms \citep{PoggioEdelman1990,Foldiak1991}.

A particularly strong empirical motivation for quantitative models of tolerance-building came from the paperclip experiments of Logothetis and colleagues \citep{Logothetis1995}. Monkeys trained to recognize novel three-dimensional objects across limited rotations in depth exhibited IT neurons that were selective for object identity and view, yet tolerant to changes in retinal position and scale. Critically, substantial scale tolerance was observed despite fixed viewing distance during training, implying that tolerance to two-dimensional transformations is not learned anew for each object but instead relies on generic invariant computations available prior to object-specific learning \citep{LogothetisSheinberg1996}. These results suggest that the ventral stream implements mechanisms that construct tolerance in a largely stimulus-independent manner.

Riesenhuber and Poggio’s HMAX model was explicitly designed to account for these properties \citep{RiesenhuberPoggio1999}. HMAX instantiates a feedforward hierarchy of alternating simple (S) and complex (C) stages, in which selectivity increases through template matching while tolerance emerges through pooling over afferent units with similar tuning. In this framework, generic invariance to position and scale is built gradually through cascaded pooling operations across layers, and higher-level units then combine these invariant features to achieve object- and view-specific selectivity. A key prediction—that complex cells implement a max-like pooling operation—was later supported by intracellular recordings showing that complex-cell responses are well described by max-like nonlinearities over simple-cell inputs \citep{Lampl2004}. Subsequent work extended HMAX into quantitatively specified ventral-stream models and demonstrated that feedforward implementations could account for both neural responses and human performance in rapid visual categorization tasks \citep{Serre2007pami,Serre2007pnas,Thorpe1996}.


The deep learning revolution in vision enabled dramatic gains in object recognition performance and spurred architectural innovations such as VGG, Inception, and residual connections \citep{Krizhevsky2012,Russakovsky2015,Simonyan2015,Szegedy2015,He2016}. For neuroscience, these advances helped establish a goal-driven, task-optimized modeling strategy: optimize a network to solve an ethologically relevant recognition task and then test its internal representations against neural measurements. Seminal work showed that such task-optimized networks could predict responses in macaque IT and V4, often outperforming earlier biologically inspired models \citep{Yamins2014,Cadieu2014}, and this relationship was later formalized in standardized benchmarks such as Brain-Score \citep{Schrimpf2020-az}.

A substantial body of work has explicitly examined invariance properties in deep neural networks. Early studies introduced quantitative metrics to measure how learned representations change under identity-preserving transformations, showing that invariance tends to increase with depth but remains modest and strongly dependent on training objectives and data \citep{Goodfellow2009}. Subsequent work demonstrated that convolution and pooling alone do not guarantee broad translation or scale invariance in practice: performance often degrades outside the range of transformations encountered during training, although targeted augmentation can induce tolerance over a limited range \citep{BiscioneBowers2021}. Analyses of modern ImageNet-trained networks similarly show that scale invariance increases with depth but remains incomplete, with intermediate layers retaining substantial scale sensitivity \citep{Graziani2021,Cui2020}. Together, these findings indicate that contemporary CNNs can exhibit nontrivial invariance, but that its extent and form depend critically on architecture, training regime, and the transformations considered.

These developments raise a fundamental question for neuroscience: when a model achieves high benchmark performance and strong neural predictivity, what aspects of its internal computation are genuinely explanatory of ventral-stream processing? Benchmark accuracy and neural predictivity do not uniquely determine mechanism. Scale tolerance provides an informative lens on this problem. Humans can generalize across substantial changes in retinal size after minimal exposure \citep{Han2020}, whereas modern convolutional networks often rely on extensive scale augmentation during training and exhibit sharp performance degradation outside the augmentation range \citep{Azulay2019why}. At the neural level, mid-level ventral-stream representations exhibit transformation-tolerant codes that remain stable across changes in retinal size, including curvature-based invariances in area V4 \citep{ElShamayleh2016Curvature}.

Here we test whether scale tolerance can serve as a mechanistic probe of ventral-stream models at modern performance levels. We systematically modernize the HMAX framework while preserving its tolerance-building organization and compare it to standard convolutional networks across object recognition, behavioral generalization, and neural alignment.
...
Models were trained using supervised learning for object categorization, with cross-entropy loss optimized via stochastic gradient descent--based methods. To isolate architectural contributions to scale tolerance, HMAX-family models were trained without extensive scale jitter, whereas baseline convolutional networks were trained using conventional augmentation practices unless explicitly stated. 
...
densely at quarter-octave increments (scale factor $2^{1/4}$), enabling smooth transitions across
scale space.
...
\cite{Schrimpf2020-em,Majaj2015-bb}. For each model layer $l$, a linear mapping from activations
...
to the full model input size to mimic foveal presentation (RF sizes computed analytically; see \cite{araujo2019computing} in the student draft).
