...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{Figures/Figure_1_v6.jpg}
...
Our dataset exhibits significant imbalance, comprising 34,328 extant and 3,200 fossil leaves—approximately a 10:1 ratio. Moreover, fossil leaves often differ markedly from extant specimens due to damage, partial preservation, compression artifacts, and rock matrix that can obscure fine venation patterns. We hypothesized that these differences might lead the model to treat extant and fossil leaves as distinct domains. To investigate this, we computed average within-family distances across both domains. Fossil leaves  and extant leaves  are generally cluster together regardless of their family. A pattern evident in the UMAP visualization of the neural network's penultimate layer embeddings (Fig.~\ref{fig:sytem}), highlighting the domain shift between extant and fossil specimens.
...
\paragraph{Concept Quantification}
Having decomposed the neural representation into interpretable concepts, we address the question of quantifying their individual contributions to model predictions. The sparse dictionary learning framework yields a linear reparameterization that renders explicit the relationship between concept activations and classification decisions.
%
