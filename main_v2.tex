...
\author*[4]{\fnm{Peter} \sur{Wilf}}\email{pwilf@psu.edu}
...
\affil[2]{\orgdiv{Kempner Institute}, \orgname{Harvard University}, \orgaddress{ \city{Boston}, \state{Massachusets}, \country{USA}}}
...
\abstract{
Accurately identifying fossil angiosperm leaves remains one of paleobotany's most persistent challenges. Although the morphological complexity of leaves has historically hindered manual classification, artificial intelligence (AI) is well-suited to extract subtle diagnostic patterns that may elude human perception. However, applying standard AI approaches to fossil material faces a fundamental limitation. The extreme scarcity of taxonomically vetted fossil specimens precludes conventional supervised training at scale.

Here, we introduce a deep learning framework that overcomes this challenge by augmenting sparse fossil data with synthetic examples and aligning extant and fossil leaf domains through representational learning. Our approach synthesizes high-fidelity fossil analogs from modern leaf images (cleared and X-rayed) and trains a deep neural network using multi-scale representations with triplet-loss embedding to align the two domains. We evaluate the system on the late Eocene Florissant flora of Colorado, achieving 91.8\% top-5 F1-score (chance: 3.5\%) for family-level classification of fossil leaves. Remarkably, it maintains 73.4\% F1-score even for fossil families excluded from training—demonstrating robust out-of-distribution generalization. To interpret the model's decision process, we developed a concept-based interpretability framework that extracts and localizes visual features driving classification decisions. We demonstrate practical utility by applying our system to 1,723 previously unidentified leaf fossils from the Florissant Formation, providing predictions with visual explanations to guide expert review. 

By overcoming data scarcity through generative AI and representational learning at Florissant, this work offers a pathway to unlock large-scale paleobotanical ``dark data"—the vast collections of unidentified specimens in museum drawers worldwide.
}
\maketitle


\section*{Introduction}
Isolated leaves dominate the angiosperm fossil record yet remain notoriously difficult to identify accurately, representing paleobotany’s largely untapped source of “dark data”~\cite{Dilcher_1971}. Historical literature is riddled with botanically incorrect identifications due to the inherent complexity of leaf morphology, insufficient vetted reference samples, and considerable variation within and across fossil sites. Consequently, most well-identified fossil leaves represent a handful of morphologically distinctive families that are well-represented in the literature, leaving the vast majority of angiosperm diversity in the fossil record unrecognized ~\cite{Hickey1975, Wilf2008FossilLeaves}.  Accurate fossil leaf classification is crucial because leaf fossils provide essential data for evolutionary radiations and extinctions, biome evolution, plant–animal interactions, biogeography, and biotic responses to climate change~\cite{Giraldo2025, Hickey1977, Johnson1989, Kooyman2014, Wing2005}. The family level, targeted here, is the traditional first step for most extant and especially fossil plant identifications because nearly all plant fossils represent extinct species and many belong to extinct genera.

Artificial intelligence (AI) shows promise for automated classification of plants and palynomorphs~\cite{plantclassificationGunjal2024,machinelearningspecies,Adaime2024}, but faces the same bottlenecks as human experts: small labeled sample sizes and strong site-to-site taphonomic variation. To control these factors, we focus on the late Eocene Florissant flora of Colorado, a uniquely well-documented lacustrine assemblage~\cite{Harding2000} rooted in MacGinitie’s monograph~\cite{mcginitie1953fossil} and subsequent revisions~\cite{manchester1983fagopsis,Manchester2001Florissant,Jia2014Cercis,Herendeen2019Arcoa}. From Meyer’s public digital archive~\cite{florissant}, recently recompiled with cleared, X-rayed, and fossil leaves~\cite{Wilf2024LeafDatasetV2}, we extracted 3{,}200 vetted Florissant leaves spanning 23 families (16 families with ≥5 specimens). A further  1{,}723 unvetted   Florissant specimens that lack confident family assignment under modern standards, were used as test to check validity of the predictions from our model. 

We focused on the late Eocene Florissant flora of Colorado, one of the best understood Cenozoic fossil plant assemblages, preserved under relatively consistent lacustrine conditions ~\cite{Harding2000} that yielded a large suite of visually similar rock matrix and leaf impressions. The Florissant flora  was systematically described in MacGinitie's seminal 1953 monograph~\cite{mcginitie1953fossil} and has been expanded and revised ever since \cite{manchester1983fagopsis,Manchester2001Florissant,Jia2014Cercis,Herendeen2019Arcoa}. Meyer's digital archive~\cite{florissant} comprises thousands of specimens and represents one of the earliest large-scale paleobotanical image datasets available online. From this resource, recently recompiled \citep{Wilf2024LeafDatasetV2} as part of an openly accessible compilation of fossil, cleared, and X-rayed leaves now totaling 34,368 leaf images~\cite{Wilf2024LeafDatasetV2}, we extracted 3,200 vetted Florissant leaves covering 23 plant families, including 16 families with at least five specimens each. These vetted fossils were selected from a larger Florissant dataset containing over 1,723 additional specimens that could not be confidently assigned to the family level under modern standards. 

To address the fundamental limitation that many fossil leaves belong to families with few or no vetted fossil training exemplars, we adopt a generative domain-translation approach to synthesize fossil-like images from extant cleared/X-rayed leaves. Specifically, we use a cycle-consistent, ControlNet-based model to style-translate cleared/X-rayed leaves from 142 modern families (each with ≥25 specimens) into fossil-like images, and we include vetted Florissant fossil photographs from 16 families to anchor the mapping between domains. This approach substantially increases the number of angiosperm families available as ``training fossils,'' expanding the effective training set (Fig.~\ref{fig:system}). We train the classifier jointly on (i) extant cleared/X-rayed images, (ii) real Florissant fossils where available, and (iii) synthetic fossils, and we find that adding synthetic fossils substantially improves family-level F1-score for families both with and without real fossil training images (Fig.~\ref{fig:system}), demonstrating that generative augmentation can overcome inherent data scarcity in macrofossil paleobotany.

\section*{Results}

We developed our deep learning architecture through multiple iterations and cumulative improvements built upon a ResNet-101 backbone~\cite{resnetHe2015}, a widely used 101-layer convolutional neural network with strong performance across computer vision tasks. Very similar results were obtained with a transformer architecture (See Table~\ref{tab:beit}) Throughout this work, we report the top-5 F1-F1-score of our deep learning model, i.e., the proportion of test images for which the correct family label appears among the five highest-ranked predictions. We chose this metric because it is more informative than simple F1-score in a multilabel setting with 142 angiosperm families, where partial correctness is common and exact match F1-score can underestimate performance. The chance-level performance for this classification task is 3.5\%. Importantly, we also evaluated model performance in terms of error rate, and found the results to be qualitatively similar, supporting the robustness of our conclusions. All experiments used (80\%, 10\%, 10\%) train, validation, and test splits, respectively. We systematically tried different model settings (grid search) and selected the best using validation data. All F1-score measures reflect the model F1-score on the test set averaged in independent stratified random splits, $n = 10$.

Evaluating fossil-specific performance presents a key challenge: our dataset contains vetted leaf-fossil specimens for only 16 of the 142 angiosperm families. To quantify classification F1-score specifically for fossil leaves, we restricted our test set to these 16 families while maintaining classification across all 142 families, as a baseline we are only considering the dataset with real samples and without using the synthetic fossils.  We evaluated two scenarios to establish performance bounds. In the \textbf{in-distribution (ID)} scenario, real fossil specimens from the test family were included during training, with evaluation performed on unseen real specimens of the same family using standard train/validation/test splits. This yielded an average test F1-score of 67.3\%, which constitutes the baseline F1 score for our architecture. 

In the more challenging \textbf{out-of-distribution (OOD)} scenario, we adopted a leave-one-family-out protocol where all real fossils from one family were withheld during training, and the model was tested on the complete set of real fossil leaves from that excluded family. As a baseline, we trained and test only on real examples.  This OOD evaluation yielded an overall F1-score of 3.82\% on test fossils, reflecting the considerably greater difficulty of generalizing from extant cleared and X-rayed images to fossil leaf images (Fig.~\ref{fig:system}).

A common challenge in deep learning is the tendency for neural networks to rely on spurious correlations—so-called shortcuts—between image backgrounds and text annotations and class labels. This issue is particularly problematic in scientific applications, where hidden data acquisition biases, annotation artifacts such as handwriting patterns or label sizes
, or preparation procedures can lead models to overfit to unintended cues rather than meaningful biological features~\citep{Brown2023ShortcutTesting,Nauta2022SkinShortcut,Hill2024ShortcutRisk}. 

\begin{figure}[htbp]
    \centering\includegraphics[width=0.8\linewidth]{Figures/Figure_1_v8.png}
    \caption{Improvements for fossil leaf family identification and their cumulative effects on classification performance.
    \textbf{Performance bounds.} Bar chart shows Top-5 ($F_1$) F1-score scores under two generalization scenarios: ``in-distribution (ID)'' using real fossil leaves during training and ``out-of-distribution (OOD)'' using only synthetic fossils during training (simulated-to-real). Chance performance for 142 families is 3.5\% (dashed black line). The final model achieves 91.8\% F1-score on combined extant and fossil leaves (excluding synthetic), with expected performance of 77.3\% (OOD) and 93.2\% (ID) on real fossils alone. \textbf{Model improvements (panels A–E):} (A) Sample raw extant leaf image; (B) automatic shortcut removal; (C) five-scale image pyramid for multi-resolution classification; (D) triplet loss to enforce joint representation for both the fossil/extinct and extant leaf domains: (left) visualization of family clusters using UMAP showing that naturally the model without any restriction cluster together by domain rather than by family; (right) representational learning (triplet loss regularization) brings samples of the same family closer regardless of the domain; (E) high-fidelity synthetic fossils are generated to augment underrepresented families and those lacking fossil specimens: (top) examples of synthetic fossil images; (bottom)  real fossil images for comparison and original extant leaf images used as source when real fossil images are unavailable.}
    \label{fig:system}
\end{figure}
...
\begin{figure}
    ...
    \caption{Sample of high level features used by the model to perform classification. Note the dentation patterns and venation patterns, that are characteristic of different families.}
...
\begin{figure}[t!]
    ...
    \caption{Top: Expert assessment of the system predictions. Examples of unidentified fossils, corresponding top-5 model predictions, and associated concepts used by the model to make such predictions. For instance, we find the arrangement of secondary veins, the pattern of dentation, the honeycomb pattern, and the junction between paternary and secondary veins. Please find the full release of predictions at \url{https://serre-lab.github.io/prj_fossil_unknown/}    }
    \label{fig:unknown}
\end{figure}
...
To facilitate broader accessibility and practical use of our system, we developed an interactive web application available at \url{https://huggingface.co/spaces/Serrelab/fossil_app}. This demo enables users to upload fossil leaf images and obtain top family-level predictions generated by our model. In addition to classification results, the application provides saliency maps that visually highlight the regions of the leaf image most influential to the model's decision, offering an interpretable and user-friendly interface for exploring fossil leaf identification.
...
Additionally, we applied the fine-tuned SAM model during synthetic image generation, ensuring that leaf shapes in synthetic images remain faithful to their input counterparts, thereby improving both visual realism and model generalization, please find details for training in (SI: Details for training) . 
...
This work was funded by an NSF FRES grant (EAR-1925481 to T.S. and EAR-1925755 to P.W.). We thank Jacob Rose for technical assistance. Computing support was provided by the Center for Computation and Visualization (CCV) (via NIH Office of the Director grant S10OD025181). We also acknowledge the Cloud TPU hardware resources that Google graciously makes available via the TensorFlow Research Cloud (TFRC) program. We thank Edward Spagnuolo, Teng-Xiang Wang, L. Alejandro Giraldo, and Steven Manchester for helpful discussions on the paleobotanical aspects of this work. Many thanks to Yuxian Wang who developed the gradio app that will help to host the models and allow people around the world to test our system in their samples. Also Paula Vargas who helped in final editing of the figures of this paper. 
...
Find Examples of more concepts that our model finds below, and please Please visit our website \url{https://fel-thomas.github.io/Leaf-Lens/} for an exhaustive list of concepts.  
\begin{figure}
    \centering
