...
\begin{abstract} \bfseries \boldmath
    Fossil angiosperm leaves comprise abundant, untapped data about evolutionary history and ancient ecosystems. However, identifying these specimens has remained a manual, error-prone, expert-dependent process, and most collections still represent paleobotanical ``dark data.'' The extreme scarcity of taxonomically vetted fossil specimens, the total lack of vetted fossils for most plant families, and preservation artifacts from fossilization have thwarted conventional machine learning approaches. Here, we address these challenges with a deep learning system that learns to generate fossil-like images from modern leaves, while simultaneously aligning family-diagnostic features across modern and fossil specimens. Tested on ``dicot'' families with vetted leaf fossils from the Florissant Fossil Beds (late Eocene, Colorado), the system maintains strong top-5 accuracy on fossils (93.2\%, vs.\ 3.5\% chance) even when all fossils from a test family are withheld during training (77.3\%). Concept-based explanations highlight novel diagnostic morphology, including venation details, margin features, and leaf base character, to guide expert review and taxonomic analysis. Applied to a set of previously unidentified Florissant specimens, our system provides credible family-level hypotheses for 85.6\% of specimens, demonstrating practical decision support for fossil leaf identification at a well-characterized site.
\end{abstract}
...
Our system integrates four components trained end-to-end: (i) a multi-scale ResNet-101 classifier that processes each leaf at five resolutions, capturing both overall shape and fine venation; (ii) automated leaf segmentation via a fine-tuned Segment Anything Model (SAM) to remove background artifacts; (iii) a ControlNet-based diffusion model that generates realistic synthetic fossils from modern cleared leaves, providing training data for families lacking real fossil specimens; and (iv) a triplet-loss objective that aligns fossil and modern leaf representations in embedding space. The system was trained on 32,913 modern cleared and X-rayed leaves from 142 families and 2,923 vetted Florissant fossils from 16 families. Interpretability analyses used sparse dictionary learning to extract visual concepts from the model's internal representations, with concept importance quantified via a reparameterized classifier. Full details are provided in the supplementary materials.
